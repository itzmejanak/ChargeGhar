...
def process_pending_refunds(self):
            ...
            try:
                ...
                if success:
                    ...
                    refund.gateway_reference = (
                        f"REF_{refund.id}_{timezone.now().strftime('%Y%m%d%H%M%S')}"
                    )
    ...
    def _process_gateway_refund(self, refund: Refund) -> bool:
        ...
        try:
            ...
            if transaction.payment_method_type in ["WALLET", "POINTS"]:
                self.logger.info(
                    f"Refund {refund.id} is for wallet/points payment, no gateway verification needed"
                )
                return True
            ...
            if not gateway_reference:
                self.logger.error(
                    f"No gateway reference found for transaction {transaction.transaction_id}"
                )
                return False
            ...
            if gateway_reference.startswith("khalti_"):
                ...
                if (
                    ...
                    and "gateway" in transaction.metadata
                ):
                    gateway = transaction.metadata.get("gateway")

            if not gateway:
                self.logger.error(
                    f"Could not determine gateway for transaction {transaction.transaction_id}"
                )
                return False
    ...
    def _process_khalti_refund(self, transaction, refund):
        ...
        try:
            ...
            self.logger.info(
                f"Processing Khalti refund for transaction {transaction.transaction_id}"
            )
    ...
    def _process_esewa_refund(self, transaction, refund):
        ...
        try:
            ...
            self.logger.info(
                f"Processing eSewa refund for transaction {transaction.transaction_id}"
            )
...
def cleanup_old_payment_data(self):
    ...
    try:
        ...
        return {"deleted_intents": deleted_intents}
...
def sync_wallet_balances(self):
    ...
    try:
        ...
        self.logger.info(
            f"Wallet balance sync completed. {len(discrepancies)} discrepancies found"
        )
        return {
